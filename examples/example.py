...
for participant in participants:
    ...
    try:
        ...
        results = pd.concat([results, pd.DataFrame(
            [[participant, data.Timestamp[0], res.x[0], res.x[1], res.x[2],
              res.x[3], res.x[4], res.x[5], r2]],
            columns=["Participant", "Start", "Phase", "Baseline", "Height",
                     "Width", "Skewness", "Bimodality", "R2"]
        )], ignore_index=True)

        # NOTE: the phase parameter is the parameter 'phi' of the fitted
        ...
        plt.title(f"Melatonin data and fitted {func_name_cap} curve, "+
                  f"R2={r2:.3f}, start: {data.Timestamp[0]}")
        plt.legend(["Melatonin data",f"{func_name_cap} curve"])
